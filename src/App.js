      ...
      for(let j=0;j<stores.length;j++){
          ...
          for(let k =0;k<stores[j].items.length;k++){
            if(stores[j].items[k].name === name){
              orgPrice = stores[j].items[k].price;
              price = (stores[j].items[k].price * stores[j].items[k].discount)/100
  ...
  <tbody>
    ...
    <div className="row row-cols-1 row-cols-lg-3 align-items-stretch g-4 py-5">
      ...
      {recipies.map((recipie, index) =>(
       <Recipies key={index} id={index} recipie={recipie} findStore={findStore} />
      ))}
